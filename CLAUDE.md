...
- **Web Analytics** is enabled from the dashboard (Analytics → Web Analytics → add `congtay.com`, auto-install for proxied site) — the API token lacks RUM permission, so it can't be scripted.
